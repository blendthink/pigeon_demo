...
interface MessageHostApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: MessageHostApi?) {
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.MessageHostApi.getMessage", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            var wrapped: List<Any?>
            try {
              wrapped = listOf<Any?>(api.getMessage())
...
class MessageFlutterApi(private val binaryMessenger: BinaryMessenger) {
  ...
  fun onMessageChanged(messageArg: String, callback: () -> Unit) {
    val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.MessageFlutterApi.onMessageChanged", codec)
